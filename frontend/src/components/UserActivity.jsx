...
const UserActivity = () => {
  ...
  // Get all usernames for filter dropdown
  const usernames = ["All Employees", ...new Set(logs.map((l) => l.userName))];

  // --- Filtering Logic ---
 const filteredLogs = logs.filter((log) => {
  const matchUser = selectedUser === "All Employees" || log.userName === selectedUser;

  // Only convert if log.dateTime is valid
  let logDate = null;
  if (log.dateTime) {
  ...
  const matchStart = !startDate || (logDate && logDate >= startDate);
  const matchEnd = !endDate || (logDate && logDate <= endDate);
  ...
  switch (event) {
    case "User logged in":
      return { icon: <UserPlus size={14} />, color: "text-green-600 bg-green-50" };
    case "User logged out":
      return { icon: <UserMinus size={14} />, color: "text-red-600 bg-red-50" };
    case "User created":
      return { icon: <UserPlus size={14} />, color: "text-green-600 bg-green-50" };
    case "User details changes":
      return { icon: <UserCog size={14} />, color: "text-blue-600 bg-blue-50" };
    case "User deleted":
      return { icon: <UserMinus size={14} />, color: "text-red-600 bg-red-50" };
    case "Inbound record created":
    case "Outbound record created":
      return { icon: <PlusCircle size={14} />, color: "text-cyan-600 bg-cyan-50" };
    case "Inbound record updated":
    case "Outbound record updated":
      return { icon: <RefreshCcw size={14} />, color: "text-orange-600 bg-orange-50" };
    case "Inbound complete successfully":
    case "Outbound complete successfully":
      return { icon: <CheckCircle2 size={14} />, color: "text-emerald-600 bg-emerald-50" };
    default:
      return { icon: <AlertCircle size={14} />, color: "text-gray-600 bg-gray-50" };
  }
};
        ...
        <div className="overflow-x-auto border border-gray-200 rounded-xl shadow-sm">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr>
                  <td colSpan="5" className="text-center py-10 text-gray-500 italic">Loading logs...</td>
                </tr>
                ...
                filteredLogs.map((log) => {
                  ...
                  return (
                    <tr key={log._id} className="odd:bg-white even:bg-gray-50 hover:bg-blue-50 transition">
                      <td className="p-4 text-gray-600 whitespace-nowrap font-medium">{formattedDate}</td>
                      <td className="p-4">
